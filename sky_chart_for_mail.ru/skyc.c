...
{
    starxy ssss1;
    ssss1.x = 640*imzoom*(1 + (-y00/(-y00+pointc.y))*pointc.x);
    ssss1.y = 640*imzoom*(1 - (-y00/(-y00+pointc.y))*pointc.z);
    if(fabs(pointc.x) < 1e-10) ssss1.x=640*imzoom;
    if(fabs(pointc.z) < 1e-10) ssss1.y=640*imzoom;
    ssss1.sm= 0;
    return ssss1;
}
...
//    if (((heightc!=0)&&(heightc!=90))||((SIZEX!=800)&&(SIZEX<1200)))
    if (chosen_const<0) {
        if (((heightc!=0)&&(heightc!=90))||(SIZEX<2048))
        {
            gdImageDestroy(im);
            im = gdImageCreateTrueColor(SIZEX, SIZEY);
