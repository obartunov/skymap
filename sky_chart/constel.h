...
static unsigned char *cns_namemap_ru[ NNAMES ] = {
   /*  0 */   "And: Андромеда",
   /*  1 */   "Ant: Насос",
   /*  2 */   "Aps: Райская Птица",
   /*  3 */   "Aql: Орел",
   ...
   /* 22 */   "Cnc: Рак",
   /* 23 */   "Col: Голубь",
   /* 24 */   "Com: Волосы Вероники",
   /* 25 */   "CrA: Южная Корона",
   /* 26 */   "CrB: Северная Корона",
   /* 27 */   "Crt: Чаша",
   /* 28 */   "Cru: Южный Крест",
   /* 29 */   "Crv: Ворон",
   /* 30 */   "Cyg: Лебедь",
   ...
   /* 77 */   "Tau: Телец",
   /* 78 */   "Tel: Телескоп",
   /* 79 */   "TrA: Южный Треугольник",
   /* 80 */   "Tri: Треугольник",
   /* 81 */   "Tuc: Тукан",
   /* 82 */   "UMa: Большая Медведица",
   /* 83 */   "UMi: Малая Медведица",
   /* 84 */   "Vel: Паруса",
   /* 85 */   "Vir: Дева",
   /* 86 */   "Vol: Летучая Рыба",
   /* 87 */   "Vul: Лисичка",
...
/* Returns 0 for match */
int
cns_check(r, d, e, tol, cnsid)
double r;
double d;
double e;
double tol; /* tolerance in radians */
int cnsid; /* constellation ID */
{
   return ((fabs(d)< 3.1415926535897932/2.0) &&
       (
        cns_pick(r,d,e) == cnsid ||
        cns_pick(r+tol/cos(d),d,e) == cnsid ||
        cns_pick(r-tol/cos(d),d,e) == cnsid ||
        cns_pick(r,d+tol,e) == cnsid ||
        cns_pick(r,d-tol,e) == cnsid ||
        cns_pick(r+tol/cos(d),d+tol,e) == cnsid ||
        cns_pick(r-tol/cos(d),d-tol,e) == cnsid ||
        cns_pick(r-tol/cos(d),d+tol,e) == cnsid ||
        cns_pick(r+tol/cos(d),d-tol,e) == cnsid
       )) ? 0 : -1;
 ...
 */
int cns_figure (id, ra, dec, dcodes)
int id;
double ra[];
double dec[];
int dcodes[];
{
	...
	for (cfp = figmap[id]; cfp->drawcode >= 0; cfp++) {
	    *ra =  (double)cfp->ra/1800.0;
	    *dec = (double)cfp->dec/60.0;
